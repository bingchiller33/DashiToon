...
export default function Home() {
    // const rs = await fetch(`${env.getBackendHost()}/api/WeatherForecasts`);
    // const data = await rs.text();

    useEffect(() => {
        const rs = fetch(`${env.getBackendHost()}/api/WeatherForecasts`, {
            credentials: "include",
        })
            .then((x) => x.text())
    ...
    return (
        <main className="flex min-h-screen flex-col items-center justify-between p-24">
            <h1>{MessageService.get("6001", ["email"])}</h1>
            <button onClick={() => ToastService.showToast("6001", ["email"])}>
                asdasdsad
            </button>
            ...
            <form>
                <input type="email" style={{ color: "black" }} />

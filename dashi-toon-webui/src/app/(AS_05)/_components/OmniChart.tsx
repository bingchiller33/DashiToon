...
import { Analytics, ChapterRanking } from "@/utils/api/analytics";
...
export function OmniChart(props: OmniChartProps) {
    const {
        data,
        ranking,
        rankingCurrentPage,
            ...
            <Card className="max-w-[100vw] flex-grow">
                ...
                <CardContent className="gap-2 md:flex">
                    <div className="flex items-center">
                        {data ? (
                            <OmniPieSubChart current={current} compare={compare} data={data.topChapters} />
                        ) : (
                                ...
                                <Table>
                                    ...
                                    >
                                        <TableRow>
                                            ...
                                            <TableHead className="min-w-[180px] text-neutral-300">Tên chương</TableHead>
                                            <TableHead className="text-neutral-300">Tập</TableHead>
                                        ...
                                        {ranking.map((sub) => (
                                            <TableRow key={sub.id}>
                                                ...
                                                <TableCell className="text-neutral-100">
                                                    <Link
                                                        href={`/author-studio/series/1/${sub.volumeId}/${sub.id}`}
                                                        className="underline hover:text-blue-300"
                                                    >
                                                        Ch. {sub.chapterNumber}: {sub.name}
                                                    </Link>
                                                </TableCell>
                                                <TableCell className="text-neutral-100">
                                                    <Link
                                                        href={`/author-studio/series/1/${sub.volumeId}`}
                                                        className="underline hover:text-blue-300"
                                                    >
                                                        {sub.volumeId}
                                                    </Link>
